...
import {
  getPeriodAnalytics, getMarketerStats,
  getSystemHealth,    generateReport,
  getSystemErrors,    clearSystemErrors,
} from "../controllers/adminController.js";
...
router.get("/live", (req, res, next) => {
  ...
  if (!rawToken) return res.status(401).json({ message: "Not authenticated" });
  ...
  try {
    ...
    console.warn("[admin/live] non-user token blocked — returning 204 to stop retry");
    return res.status(204).end();
  }
}, requireAdmin, getLiveAnalytics);
