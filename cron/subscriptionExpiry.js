...
async function checkExpiringSubscriptions() {
  ...
  for (const daysLeft of NOTIFY_AT_DAYS) {
    ...
    console.log(
      `[cron] ${expiringUsers.length} vendors notified for ${daysLeft}-day expiry`,
    );
  }
  ...
  for (const user of expiredUsers) {
    ...
    await User.findByIdAndUpdate(user._id, { role: "guest" });

    // Reset planPriority on all of this vendor's active ads to 0 (bottom of feed)
    // so they sort last. We do NOT set isActive:false — expired ads stay visible
    // at the bottom of the feed as a passive renewal nudge.
    await Ad.updateMany(
      { postedBy: user._id, isActive: true },
      {
        $set: { "subscription.planPriority": 0, "subscription.plan": "Basic" },
      },
    ).catch(() => {});
    ...
    await Notification.findOneAndUpdate(
      { dedupeKey },
      {
        ...
        message:
          "Your subscription has expired. Your listings are now showing at lower priority. Renew to get top placement back.",
        actionUrl: "/subscription?renew=1",
  ...
  console.log(
    `[cron] ${expiredUsers.length} expired vendors downgraded, planPriority reset to 0`,
  );
}
...
async function cleanStalePendingPurchases() {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(
        `[cron] Cleaned ${result.deletedCount} stale pending purchases`,
      );
    }
  } catch (err) {
...
export function startSubscriptionCron() {
  ...
  console.log(
    "[cron] Subscription expiry + stats (08:00 WAT) + pending cleanup (every 2h) scheduled",
  );
}
