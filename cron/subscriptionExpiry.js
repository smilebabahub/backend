...
async function checkExpiringSubscriptions() {
  ...
  for (const user of expiredUsers) {
    ...
    await Notification.findOneAndUpdate(
      { dedupeKey },
      {
        ...
        message:
          "Your subscription has expired. Your listings are paused. Renew to reactivate them.",
        actionUrl: "/subscription?renew=1",
  ...
  console.log(
    `[cron] ${expiredUsers.length} expired vendors downgraded to guest`,
  );
}
...
export function startSubscriptionCron() {
  ...
  console.log(
    "[cron] Subscription expiry + daily stats scheduled (daily 08:00 WAT)",
  );
}

export { checkExpiringSubscriptions, writeDailyStats };
