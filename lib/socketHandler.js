  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data) => {
      try {
        ...
        // ── Email notification (debounced, only when receiver offline) ────
        maybeNotifyByEmail({
          receiverId: receiver,
          senderId: sender,
...
// ── Push a socket event to a specific user if they're online ──────────────
export function notifyUser(io, userId, event, data = {}) {
  const socketId = onlineUsers.get(String(userId));
  if (socketId) io.to(socketId).emit(event, data);
}

// ── Singleton io reference — set by server.js after io is created ─────────
// Controllers import pushToUser from here instead of socketEmitter.js
// so there is no separate file to go missing.
let _io = null;
