...
/** Topics that shouldn't sit in a queue overnight. */
const URGENT_TOPICS = ["money", "report"];
...
export async function sendContactEmails({
  ...
  const urgent = URGENT_TOPICS.includes(topic);
  ...
  await send({
    ...
    html: baseTemplate({
      ...
      previewText: `Your reference is ${reference}. We'll reply within ${urgent ? "12" : "24"} hours.`,
      body: `
        ${heading("Message received")}
        ${subheading(`Hi ${displayName.split(" ")[0]}, thanks for getting in touch.`)}
 
        <p style="font-size:15px;color:${BRAND.textDark};line-height:1.7;margin:0 0 20px;">
          ${
            urgent
              ? "Because this involves money or a safety report, it's been prioritised. Expect a reply within <strong>12 hours</strong>."
              : "We'll reply to this address within <strong>24 hours</strong>."
          }
        </p>
        ...
        ${infoBox([
          [
            ...
            `<span style="font-family:monospace;font-weight:900;
              color:${BRAND.yellow};">${reference}</span>`,
          ],
          ["Topic", topicLabel ?? topic ?? "General"],
          ...(orderReference ? [["Order/transfer", orderReference]] : []),
          ["Received", new Date().toLocaleString("en-GH")],
        ])}
 
        <div style="background:${BRAND.gray};border-radius:12px;padding:16px 20px;margin:0 0 8px;">
          <p style="margin:0 0 8px;font-size:11px;font-weight:700;
            text-transform:uppercase;letter-spacing:0.05em;color:${BRAND.textMuted};">
            What you sent us
          </p>
          <p style="margin:0;font-size:13px;color:${BRAND.textDark};
            line-height:1.6;white-space:pre-wrap;">${escapeHtml(message)}</p>
        </div>
 
        ${divider()}
        <p style="font-size:13px;color:${BRAND.textMuted};line-height:1.6;margin:0;">
          You can reply to this email to add anything you've forgotten —
  ...
  await send({
    to: ADMIN(),
    replyTo: email,
    subject: `${urgent ? "[URGENT] " : ""}${topicLabel ?? "Contact"} — ${displayName} — ${reference}`,
    html: baseTemplate({
      title: "New contact message",
      previewText: `${displayName}: ${message.slice(0, 90)}`,
      body: `
        ${adminBadge()}
        ${heading(urgent ? "Urgent contact message" : "New contact message")}
        ${subheading(`${displayName} has been in touch. Reply to this email to answer them directly.`)}
        ...
        <div style="background:${BRAND.gray};border-radius:12px;padding:16px 20px;">
          <p style="margin:0;font-size:14px;color:${BRAND.textDark};
            line-height:1.7;white-space:pre-wrap;">${escapeHtml(message)}</p>
        </div>
 
        ${ctaButton("Open admin inbox", `${APP}/admin/support`)}
      `,
    }),
  });
}
...
// 6. ACCOUNT DELETION
//
// Three emails across the lifecycle. The confirmation one matters most —
// without it, anyone who knows an email address could close the account
// behind it.
// ═══════════════════════════════════════════════════════════════════════
...
export async function sendDeletionConfirmEmail({
  ...
  await send({
    to: email,
    subject: "Confirm your SmileBaba Hub account deletion",
    html: baseTemplate({
      title: "Confirm account deletion",
      previewText: "Nothing has been deleted yet. Confirm to continue.",
      body: `
        ${heading("Confirm account deletion")}
        ${subheading(`Hi ${username || "there"}, we received a request to delete your account.`)}
 
        <div style="background:#FEF3C7;border:1px solid ${BRAND.yellow};
          ...
          <p style="margin:8px 0 0;font-size:13px;color:#B45309;line-height:1.6;">
            We need you to confirm first, so nobody else can close your account.
          </p>
        </div>
 
        <p style="font-size:15px;color:${BRAND.textDark};line-height:1.7;margin:0 0 4px;">
          If this was you, confirm below. This link expires in 48 hours.
        </p>
 
        <div style="text-align:center;margin:28px 0 0;">
          <a href="${confirmUrl}" style="display:inline-block;background:#DC2626;
            color:${BRAND.white};font-weight:900;font-size:15px;padding:14px 36px;
            border-radius:50px;text-decoration:none;letter-spacing:0.3px;">
            Confirm account deletion
          </a>
        </div>
 
        <p style="font-size:13px;color:${BRAND.textMuted};line-height:1.6;
          margin:24px 0 0;text-align:center;">
          Your account is then deleted after <strong>${graceDays} days</strong>.
          Signing in before that cancels the request.
        </p>
 
        ${divider()}
        <p style="font-size:13px;color:${BRAND.textDark};line-height:1.6;margin:0 0 12px;">
          <strong>If this wasn't you</strong>, ignore this email — nothing will
          happen. You may also want to change your password.
        </p>
        <p style="font-size:12px;color:${BRAND.textMuted};line-height:1.6;margin:0;">
          Some records are kept after deletion where the law requires it.
          <a href="${APP}/legal/privacy#account-deletion"
            style="color:${BRAND.yellow};font-weight:700;">Read why &rarr;</a>
        </p>
      `,
    }),
  });
 
  // Privacy team gets a heads-up so nothing lands unnoticed
  await send({
    to: process.env.PRIVACY_EMAIL || ADMIN(),
    subject: `Deletion request — ${email}`,
    html: baseTemplate({
      title: "Account deletion requested",
      previewText: `${email} requested deletion. Awaiting their confirmation.`,
      body: `
        ${adminBadge()}
        ${heading("Account deletion requested")}
        ${subheading("Awaiting the user's email confirmation. No action needed yet.")}
        ${infoBox([
          ["User", username || "—"],
          ["Email", email],
          ["Grace period", `${graceDays} days after confirmation`],
          ["Time", new Date().toLocaleString("en-GH")],
        ])}
      `,
    }),
...
export async function sendDeletionScheduledEmail({
  ...
  const when = new Date(scheduledFor).toLocaleDateString("en-GH", {
    dateStyle: "long",
  });
 
  await send({
    to: email,
    subject: "Your SmileBaba Hub account will be deleted",
    html: baseTemplate({
      title: "Deletion confirmed",
      previewText: `Your account will be deleted on ${when}.`,
      body: `
        ${heading("Deletion confirmed")}
        ${subheading(`Hi ${username || "there"}, your request is confirmed.`)}
        ${infoBox([
          ["Account", email],
          ["Deletion date", when],
          ["Status", "Scheduled"],
        ...
        <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;
          ...
          <p style="margin:8px 0 0;font-size:13px;color:#15803D;line-height:1.6;">
            Sign in before ${when} and the request is cancelled automatically.
            Nothing else to do.
          </p>
        </div>
        ${ctaButton("Sign in to cancel", `${APP}/login`)}
      `,
    }),
  });
}
...
export async function sendDeletionCompleteEmail({
  ...
  const notes = retentionNotes.length
    ? `
      <div style="background:${BRAND.gray};border-radius:12px;padding:16px 20px;margin:0 0 24px;">
        ...
        <ul style="margin:0;padding:0 0 0 18px;">
          ${retentionNotes
            .map(
              (n) =>
                `<li style="font-size:13px;color:${BRAND.textDark};padding:3px 0;">${escapeHtml(n)}</li>`,
            )
            .join("")}
        </ul>
        <p style="margin:12px 0 0;font-size:12px;color:${BRAND.textMuted};line-height:1.6;">
          These hold only what's legally necessary and aren't used for
          anything else.
        </p>
      </div>`
    : "";
  ...
  await send({
    to: email,
    subject: "Your SmileBaba Hub account has been deleted",
    html: baseTemplate({
      title: "Account deleted",
      previewText: "Your account and personal information have been removed.",
      body: `
        ${heading("Your account has been deleted")}
        ${subheading("Your personal information has been removed as requested.")}
        ${notes}
        <p style="font-size:14px;color:${BRAND.textDark};line-height:1.7;margin:0 0 24px;">
          Thanks for having been part of SmileBaba Hub. You're welcome back
          any time — a new account starts fresh.
        </p>
        ${divider()}
        <p style="font-size:12px;color:${BRAND.textMuted};line-height:1.6;margin:0;">
          Questions about your data? Email
          <a href="mailto:privacy@smilebabahub.com"
            style="color:${BRAND.yellow};font-weight:700;">privacy@smilebabahub.com</a>
        </p>
      `,
    }),
  });
...
export async function sendPasswordResetEmail({
  ...
  await send({
    to: email,
    subject: "Reset your SmileBaba Hub password",
    html: baseTemplate({
      title: "Reset your password",
      previewText: `This link works for ${expiresInMinutes} minutes.`,
      body: `
        ${heading("Reset your password")}
        ${subheading(`Hi ${username || "there"}, we got a request to reset your password.`)}
 
        <p style="font-size:15px;color:${BRAND.textDark};line-height:1.7;margin:0 0 24px;">
          Click below to choose a new one. This link works for
          <strong>${expiresInMinutes} minutes</strong> and can only be used once.
        </p>
 
        ${ctaButton("Choose a new password", resetUrl)}
 
        <p style="font-size:12px;color:${BRAND.textMuted};line-height:1.6;margin:20px 0 0;text-align:center;">
          Button not working? Paste this into your browser:<br>
          <span style="word-break:break-all;color:${BRAND.textDark};">${resetUrl}</span>
        </p>
 
        ${divider()}
 
        <p style="font-size:13px;color:${BRAND.textDark};line-height:1.6;margin:0;">
          <strong>If this wasn't you</strong>, ignore this email — your password
          stays as it is. Nobody can change it without this link.
        </p>
      `,
    }),
  });
...
export async function sendPasswordChangedEmail({ email, username, at }) {
  await send({
    to: email,
    subject: "Your SmileBaba Hub password was changed",
    html: baseTemplate({
      title: "Password changed",
      previewText: "If this wasn't you, contact us straight away.",
      body: `
        ${heading("Your password was changed")}
        ${subheading(`Hi ${username || "there"}, this is just to let you know.`)}
 
        ${infoBox([
          ["Account", email],
          ["Changed", new Date(at).toLocaleString("en-GH")],
        ])}
 
        <p style="font-size:14px;color:${BRAND.textDark};line-height:1.7;margin:0 0 20px;">
          You've been signed out everywhere else, so anyone using your account
        ...
        <div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:12px;padding:16px 20px;">
          ...
          <p style="margin:8px 0 0;font-size:13px;color:#B91C1C;line-height:1.6;">
            Someone else may have access to your email. Contact us immediately at
            <a href="mailto:support@smilebabahub.com" style="color:#991B1B;font-weight:700;">support@smilebabahub.com</a>.
          </p>
        </div>
      `,
    }),
...
// ─── util ────────────────────────────────────────────────────────────
// User-supplied text goes into HTML emails, so it has to be escaped.
