...
// ── Feed cache helpers ─────────────────────────────────────────────────────
// Used by productController / adController to cache country+category feeds.
// TTL is short (5 min) so new ads appear quickly without stale data issues.
// Cache is busted automatically when TTL expires — no manual invalidation needed
// for feeds. For single-ad updates (new post / delete) we bust by prefix.
...
// Bust all feed caches for a country when a new ad is posted or deleted.
// Pattern: feed:<country>:* — uses SCAN not KEYS to avoid blocking Redis.
